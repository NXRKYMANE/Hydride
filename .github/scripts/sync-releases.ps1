<#
  Hydride Release 资产镜像同步（GitHub → Gitee / Gitea 风格镜像）
...
function Invoke-MirrorForm([string]$method, [string]$path, $form) {
    if ($Style -eq "gitee") {
        if ($null -ne $form) {
            ...
            # 用 curl --data-binary 发送临时文件: 字节原样, 规避 PS 5.1 字符串 Latin-1 与
            # 旧版 pwsh byte[] 重编码导致 emoji 损坏/Gitee 400 的跨版本问题
...
# 把对象写入 UTF-8 临时 JSON 文件, 由 curl --data-binary 原样上传（与上传资产同一套稳妥链路）
function Invoke-MirrorJsonCurl([string]$method, [string]$uri, $form, $headers) {
    ...
    $headerArgs = @("-H", "Content-Type: application/json; charset=UTF-8")
    if ($null -ne $headers) {
        foreach ($h in $headers.GetEnumerator()) { $headerArgs += @("-H", "$($h.Key): $($h.Value)") }
    }
    try {
        $resp = & curl.exe -sS -A $BrowserUA -m 120 --retry 3 --retry-delay 5 -X $method @headerArgs `
            --data-binary "@$jsonFile" $uri 2>$null
        if ($LASTEXITCODE -ne 0) { throw "mirror JSON $method failed (curl exit $LASTEXITCODE)" }
...
function Sync-Assets($ghRelease, $mirrorRelease) {
    ...
    $tmpDir = Join-Path (Join-Path ([System.IO.Path]::GetTempPath()) "hydride-sync-cache") $ghRelease.tagName
